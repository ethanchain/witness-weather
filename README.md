# Why
This repository hosts a decentralized application that predicts tomorrow's weather in Times Square, New York. The precise location is latitude: 40.7580° N, longitude: 73.9855° W. While New York observes Daylight Saving Time (DST) starting on the second Sunday in March, we will consistently use Eastern Standard Time (EST) UTC−5 for simplicity. The weather prediction will focus on the time frame from 12:00 to 12:10 tomorrow.

After reviewing documentation from various mainstream oracle services, we found no direct implementation for transferring real-world weather conditions to the blockchain. Although Chainlink’s Data Feeds hint at such capabilities, a specific solution is not provided. As a result, we employed Chainlink’s VRF (Verifiable Random Function) to generate a random weather condition for Times Square tomorrow from 12:00 to 12:10, using the random numbers from the VRF with a modulo operation.

# Getting Started 

It's recommended that you've gone through the [hardhat getting started documentation](https://hardhat.org/hardhat-runner/docs/getting-started#overview) before proceeding here. 
...
```bash
git clone https://github.com/ethancws/witness-weather/
cd witness-weather
```
Then:
```
...
```
npm install --save-dev @chainlink/contracts @ethersproject/abi @ethersproject/providers @nomicfoundation/hardhat-chai-matchers @nomicfoundation/hardhat-network-helpers @nomicfoundation/hardhat-toolbox @nomicfoundation/hardhat-verify @nomiclabs/hardhat-ethers @nomiclabs/hardhat-etherscan @typechain/ethers-v5 @typechain/hardhat chai ethers hardhat hardhat-gas-reporter husky lint-staged prettier prettier-plugin-solidity solhint solhint-plugin-prettier solidity-coverage typechain
```

That's also the case if you are using yarn.
```
yarn add --dev @chainlink/contracts @ethersproject/abi @ethersproject/providers @nomicfoundation/hardhat-chai-matchers @nomicfoundation/hardhat-network-helpers @nomicfoundation/hardhat-toolbox @nomicfoundation/hardhat-verify @nomiclabs/hardhat-ethers @nomiclabs/hardhat-etherscan @typechain/ethers-v5 @typechain/hardhat chai ethers hardhat hardhat-gas-reporter husky lint-staged prettier prettier-plugin-solidity solhint solhint-plugin-prettier solidity-coverage typechain
```
...
This will deploy your contracts to a local network. Additionally, if on a local network, it will deploy mock Chainlink contracts for you to interact with.
...
This section of the file is where you define which networks you want to interact with. You can read more about that whole file in the [hardhat documentation.](https://hardhat.org/hardhat-runner/docs/config)
...
Head over to the [Chainlink faucets](https://faucets.chain.link/) and get some ETH and LINK. Please follow [the chainlink documentation](https://docs.chain.link/resources/acquire-link) if unfamiliar. 
...
Since all tests are written in a way to be independent from each other, you can save time by running them in parallel. Make sure that `AUTO_FUND=false` inside `.env` file. There are some limitations with parallel testing, read more about them [here](https://hardhat.org/hardhat-runner/docs/guides/test-contracts#running-tests-in-parallel)
...
As explained in the [developer documentation](https://docs.chain.link/vrf).

Read the docs first to understand which method is the most suitable for your use case.

### VRF Subscription method
To start, go to [VRF Subscription Page](https://vrf.chain.link/bnb-chain-testnet) and create the new subscription. Save your subscription ID and put it in `helper-hardhat-config.js` file as `subscriptionId`: 
...
## Automation
The AutomationCounter contract is a simple Chainlink Automation enabled contract that simply maintains a counter variable that gets incremented each time the performUpkeep task is performed by a Chainlink Automation. Once the contract is deployed, you should head to [https://docs.chain.link/chainlink-automation](https://docs.chain.link/chainlink-automation) to register it for upkeeps, then you can use the task below to view the counter variable that gets incremented by Chainlink Automation
...
To estimate gas, just set a `ENABLE_REPORT_GAS` environment variable to true, and then run:
...
Ethereum/Polygon/Avalanche/etc Address: 0x3B1Ae5DE8aad2740e93C79F7962A8973E3476F30
